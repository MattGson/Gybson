...
> {
    ...
    public constructor(params: {
        ...
        // this.schema = schema;
        // this.queryTable = new
        // this.queryTable.tableAlias = `${this.queryTable.tableName}_1`;
        this.whereResolver = new WhereResolver(schema);
        this.queryTable = new QueryTable(tableName, schema);
    }

    // private get primaryKey(): string[] {
    //     return this.schema.tables[this.queryTable.tableName].primaryKey?.columnNames || [];
    // }

    // private get aliasedTable(): string {
    //     return `${this.queryTable.tableName} as ${this.queryTable.tableAlias}`;
    // }

    // private aliasedColumn(column: string) {
    //     return `${this.queryTable.tableAlias}.${column}`;
    // }

    // private get softDeleteColumn(): ColumnDefinition | null {
    //     return this.schema.tables[this.queryTable.tableName].softDelete;
    // }

    // private get hasSoftDelete(): boolean {
    //     return this.queryTable.softDeleteColumn != null;
    // }

    // private get aliasedSoftDeleteColumn(): string | null {
    //     const column = this.queryTable.softDeleteColumn;
    //     if (!column) return null;
    //     return `${this.queryTable.aliasedColumn(column.columnName)}`;
    // }
    ...
    public async findMany(
        ...
        const query = this.knex(this.queryTable.aliasedTable).select(`${this.queryTable.tableAlias}.*`);

        if (where) {
            this.whereResolver.resolveWhereClause({
                where,
                queryBuilder: query,
                tableName: this.queryTable.tableName,
                tableAlias: this.queryTable.tableAlias,
            });
        }
        ...
        if (orderBy) {
            for (const [column, direction] of Object.entries(orderBy)) {
                query.orderBy(this.queryTable.aliasedColumn(column), direction);
            }
        }

        if (paginate) {
            const { first, after, before, offset } = paginate;

            if (first) query.limit(first);
            if (offset) query.offset(offset);
            if (after) {
                Object.entries(after).forEach(([column, value]) => {
                    query.where(this.queryTable.aliasedColumn(column), '>', value);
                });
            }
            if (before) {
    ...
    public async upsert(
        ...
        const query = this.knex(this.queryTable.tableName).insert(insertRows);
        ...
        if (mergeColumns) {
            ...
            if (reinstateSoftDeletes && this.queryTable.softDeleteColumn) {
                columnsToMerge.push(this.queryTable.softDeleteColumn.columnName);
                // add soft delete reset to all records
                insertRows = insertRows.map((value) => {
                    return {
                        ...value,
                        ...this.queryTable.softDeleteFilter(false),
                    };
                });
            }
            ...
            query.onConflict(this.queryTable.primaryKey).merge(columnsToMerge);
        }
        if (update) {
            ...
            query.onConflict(this.queryTable.primaryKey).merge(updates);
        }

        // TODO:- onConflict only works on the primary key of the table in PG, maybe need an alternative behvaiour?
        if (this.engine === 'pg') query.returning(this.queryTable.primaryKey);
        if (connection) query.connection(connection);
        ...
        if (this.engine == 'pg') return result[0][this.queryTable.primaryKey[0]];
    ...
    public async insert(
        ...
        const query = this.knex(this.queryTable.tableName).insert(values);
        if (ignoreDuplicates) query.onConflict(this.queryTable.primaryKey).ignore();
        ...
        // wrong behaviour for compound keys
        if (this.engine == 'pg') {
            if (result[0]) return result[0][this.queryTable.primaryKey[0]];
            return 0;
        }
        // MySQL seems to return 0 for non-auto-increment inserts (or compound inserts)
        return result[0];
    ...
    public async update(params: { values: PartialTblRow; where: TblWhere } & ProvideConnection): Promise<unknown> {
        ...
        const query = this.knex(this.queryTable.aliasedTable).update(values);

        if (this.engine === 'pg') {
            const innerQuery = this.knex(this.queryTable.aliasedTable).select(this.queryTable.primaryKey);
            this.whereResolver.resolveWhereClause({
            ...
            if (where) {
                query.whereIn(this.queryTable.primaryKey, innerQuery);
            }
        } else if (where) {
            this.whereResolver.resolveWhereClause({
                queryBuilder: query,
    ...
    public async softDelete(params: { where: TblWhere } & ProvideConnection): Promise<unknown> {
        ...
        if (!this.queryTable.hasSoftDelete)
            throw new Error(`Cannot soft delete for table: ${this.queryTable.tableName}`);
        if (Object.keys(where).length < 1) throw new Error('Must have at least one where condition');

        const query = this.knex(this.queryTable.aliasedTable);

        // support both soft delete types
        const colAlias = this.queryTable.softDeleteColumn!.columnName;
        const type = this.queryTable.softDeleteColumn?.tsType;
    ...
    public async delete(params: { where: TblWhere } & ProvideConnection): Promise<unknown> {
        const { where, connection } = params;
        if (this.queryTable.hasSoftDelete)
            this.logger.warn(`Running delete for table: "${this.queryTable.tableName}" which has a soft delete column`);
        ...
        const query = this.knex(this.queryTable.tableName)
            .whereIn(this.queryTable.primaryKey, function () {
    ...
    public async truncate(params: ProvideConnection): Promise<unknown> {
        ...
        this.logger.warn(
            `Running truncate for table: "${this.queryTable.tableName}". This is a dangerous operation and is rarely required in production.`,
        );

        const query = this.knex(this.queryTable.tableAlias).truncate();
